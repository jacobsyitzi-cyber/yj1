...
st.markdown(
    f"""
<style>
.stApp {{ background: {RCG_BG}; }}
.block-container {{ padding-top: 1.0rem; padding-bottom: 2rem; }}

/* Sidebar */
section[data-testid="stSidebar"] {{
  background: {RCG_PURPLE};
  border-right: 0;
}}
...
section[data-testid="stSidebar"] textarea {{
  ...
  color: {RCG_TEXT} !important;
}}
section[data-testid="stSidebar"] [data-baseweb="select"] > div {{
  background: white !important;
  color: {RCG_TEXT} !important;
}}
section[data-testid="stSidebar"] [data-baseweb="select"] * {{ color: {RCG_TEXT} !important; }}
...
[data-testid="stMetric"] {{
  background: {RCG_CARD};
  border: 1px solid {RCG_BORDER};
  border-radius: 14px;
  padding: 14px 14px 10px 14px;
  box-shadow: 0 1px 2px rgba(0,0,0,0.04);
}}
[data-testid="stMetricLabel"] {{ color: {RCG_MUTED}; }}
[data-testid="stMetricValue"] {{ color: {RCG_PURPLE}; }}

/* Tabs */
.stTabs [data-baseweb="tab"] {{ font-weight: 800; }}
.stTabs [aria-selected="true"] {{ color: {RCG_PURPLE} !important; }}

h1, h2, h3, h4 {{ color: {RCG_PURPLE}; }}
</style>
""",
...
if "__filters__" not in st.session_state:
    st.session_state["__filters__"] = {
        ...
        "branches": [], "employees": [], "channels": [], "depts": [], "customers": [], "products": [],
        "invoice": "", "returns_mode": "Include returns",
    }
...
def open_details(kind: str, value: str):
    ...
    st.session_state["active_tab_hint"] = "Details"

# ---------- Tabs ----------
tabs = ["Overview","Branches","Employees","Products","Customers","Returns","Invoices","Details","Raw data","Export"]
tab_overview, tab_branches, tab_employees, tab_products, tab_customers, tab_returns, tab_invoices, tab_details, tab_data, tab_export = st.tabs(tabs)
...
def drill_tab(kind: str, df_rank: pd.DataFrame, key_col: str, options: List[str], select_key: str):
    ...
    with c1:
        if not df_rank.empty:
            choose_chart_money(df_rank.head(20), top_chart, key_col,
                               "Total_Inc_VAT" if revenue_basis == "Total (incl VAT)" else "Total_Ex_VAT",
